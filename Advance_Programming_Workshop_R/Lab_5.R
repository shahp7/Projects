...
#the graph for the points and minutes played. The most efficent player
matplot(t(Points/MinutesPlayed), type = "b", pch=15:18, col=c(1:4,6))

legend("topleft", inset= 0.01, legend= Players, col=c(1:4,6), pch=15:18,
       horiz = F)
#the graph for the productivity vs. salary paid. The Best value

myplot<- function(z, who=1:10){
  matplot(t(z[who,,drop=F]), type="b", pch=15:18, col=c(1:4,6),
          main="Basketball Player Productivity")
  legend("bottomright", inset=0.01, legend=Players[who], 
         col=c(1:4,6), pch=15:18, horiz = F)
}

myplot(FieldGoals/FieldGoalAttempts)

myplot((Games-Points)/Salary)
