...
B<-stats[,"Internet.users"]

#what are the other factors? Levels is an bulit in rule
levels(stats$Income.Group)

# Basic operation with Data Frame
A <-stats[1:10,] #subsetting
stats[3:9,]
...
stats[,1]
is.data.frame(stats[,1])
# its not a DF anymore, it will make it as a data set
stats[,1,drop=F]
...
# math operations
D<-stats$Birth.rate * stats$Internet.users
E<-stats$Birth.rate + stats$Internet.users
...
stats$xyz <-1:4 # would not work, why? It is not an even divide of 5 numbers
...
head(stats)
#anything witha < or > it will make it a boolon. It will ask for true oor false
stats$Internet.users < 2
filer<-stats$Internet.users < 2
...
#homework answer part one
stats[stats$Birth.rate < 15,]

#One part of table for the homework. List out the countries where the the
#Birth Rate is < 15 or Internet Users > 80 (2 points)
H<-stats[stats$Birth.rate < 15 | stats$Internet.users > 80,]
qplot(data=stats, x=Internet.users, y=Birth.rate)

#Second part of table for the homework
#Display one new data frame (first 30 rows) sorted based on the Income group, 
#and followed by descending order based on Internet User columns. (3 points). 
#(hint: You need to research this on your own, and 
#you are welcome to use any functions that you found useful)
HH<-stats[1:30, stats$Income.Group & stats$Internet.users]
...
qplot(data=merged, x=Internet.users, y=Birth.rate, color=Region, 
      size=I(5), shape=I(17), alpha=I(0.1))
qplot(data=merged, x=Internet.users, y=Birth.rate, color=Region, 
      size=I(5), shape=I(17), alpha=I(0.6))

#adding title

qplot(data=merged, x=Internet.users, y=Birth.rate, color=Region, size=I(5), 
      shape=I(17), alpha=I(0.1), main="Birth Rate vs. Internet Users")